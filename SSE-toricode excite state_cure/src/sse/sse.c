    ...
    for (int p = 0; p < state->M; p++) {
        if (state->op_string[p] == 0) {
            ...
            int b = (state->op_string[p] / 17) - 1;
                ...
                system->spin[system->bond[b][4]] *= -1;
                system->spin[system->bond[b][5]] *= -1;
...
void init_toricode_system(int L, double S, toricode_system *system) 
{
    ...
    for (int i = 0; i < L; i++) {
        for (int j = 0; j < L; j++) {
            system->bond[j * L + i][0] = 2 * j * L + i;
            system->bond[j * L + i][1] = 2 * j * L + (i - 1 + L) % L;
            system->bond[j * L + i][2] = ((2 * j + 1) % (2 * L)) * L + i;
            system->bond[j * L + i][3] = ((2 * j - 1 + 2 * L ) % (2 * L)) * L + i;
            system->bond[j * L + i][4] = ((2 * (j + 1)) % (2 * L)) * L + i;
            system->bond[j * L + i][5] = ((2 * j + 1) % (2 * L)) * L + (i + 1) % L;
        }
    }
    for (int i = 0; i < system->N; i++) {
