...
void sample(int n, int t_idx, toricode_system *system, sse_state *state, sampled_quantities *samples, pcg32_random_t* rng) 
{
    ...
    for (int p = 0; p < state->M; p++) {
        if ((state->op_string[p] % 17 == 2 || state->op_string[p] % 17 == 3 ||
            state->op_string[p] % 17 == 5 || state->op_string[p] % 17 == 8 ||
            state->op_string[p] % 17 == 10 || state->op_string[p] % 17 == 12 ||
            state->op_string[p] % 17 == 14 || state->op_string[p] % 17 == 15) &&
            state->op_string[p] / 17 != 2) {
            Ov *= -1;
        }
        else if ((state->op_string[p] % 17 != 1 || state->op_string[p] % 17 != 4 ||
                state->op_string[p] % 17 != 6 || state->op_string[p] % 17 != 7 ||
                state->op_string[p] % 17 != 11 || state->op_string[p] % 17 != 13 ||
                state->op_string[p] % 17 != 16) &&
                state->op_string[p] / 17 == 2) {
            Ov *= -1;
        }
    }
