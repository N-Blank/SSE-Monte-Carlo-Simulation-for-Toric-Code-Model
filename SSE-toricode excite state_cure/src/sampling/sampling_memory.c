...
void init_samples(double *beta_vals, int len_beta, int n_bins, int L, struct sampled_quantities *samples, int max_samp) 
{
    ...
    samples->Ov_bins = (double **) malloc(len_beta * sizeof(double *));
    ...
    for (int i = 0; i < len_beta; i++) {
        ...
        samples->Ov_bins[i] = (double *) malloc(n_bins * sizeof(double));
        ...
        memset(samples->Ov_bins[i], 1.0, n_bins * sizeof(double));
    ...
    samples->Ov_mean = (double *) malloc(len_beta * sizeof(double));
    samples->Ov_std = (double *) malloc(len_beta * sizeof(double));
    ...
    memset(samples->Ov_mean, 1.0, len_beta * sizeof(double));
    memset(samples->Ov_std, 0.0, len_beta * sizeof(double));
