...
void sample(int n, int t_idx, toricode_system *system, sse_state *state, sampled_quantities *samples, pcg32_random_t* rng) 
{
    ...
    // sample the sign coefficient
    for (int p = 0; p < state->M; p++) {
        if (state->op_string[p]%17 != 0 && state->op_string[p] / 17 == 2){
            Ov *= -1;
        }
    }
 ...
 */
void normalize(long mc_cycles, sampled_quantities *samples, int N, int L, double S) 
{
    for (int t_idx = 0; t_idx < samples->betas; t_idx++) {
        for (int n = 0; n < samples->bins; n++) {
            ...
            samples->Ov_bins[t_idx][n] /= mc_cycles;
            ...
            samples->Ov_mean[t_idx] += samples->Ov_bins[t_idx][n];
        ...
        samples->Ov_mean[t_idx] /= samples->bins;
        ...
        for (int n = 0; n < samples->bins; n++) {
            ...
            samples->Ov_std[t_idx] += pow(samples->Ov_bins[t_idx][n] - samples->Ov_mean[t_idx], 2.0);
        ...
        samples->n_std[t_idx] = (samples->n_std[t_idx] / samples->bins);
        samples->E_std[t_idx] = (samples->E_std[t_idx] / samples->bins);
        samples->m_std[t_idx] = (samples->m_std[t_idx] / samples->bins);
        samples->Ov_std[t_idx] = (samples->Ov_std[t_idx] / samples->bins);
        samples->m2_std[t_idx] = (samples->m2_std[t_idx] / samples->bins);
        samples->m4_std[t_idx] = (samples->m4_std[t_idx] / samples->bins);
        samples->ms_std[t_idx] = (samples->ms_std[t_idx] / samples->bins);
        samples->m2s_std[t_idx] = (samples->m2s_std[t_idx] / samples->bins);
        samples->m4s_std[t_idx] = (samples->m4s_std[t_idx] / samples->bins);
        samples->m_sus_std[t_idx] = (samples->m_sus_std[t_idx] / samples->bins);
        samples->S_std[t_idx] = (samples->S_std[t_idx] / samples->bins);

        for (int i = 0; i < L; i++) {
            samples->corr_std[t_idx][i] = (samples->corr_std[t_idx][i] / samples->bins);
        }
        
        for (int k = 0; k < samples->k_max; k++) {
            samples->L_SS_std[t_idx][k] = (samples->L_SS_std[t_idx][k] / samples->bins);
            samples->L_HH_std[t_idx][k] = (samples->L_HH_std[t_idx][k] / samples->bins);
            samples->L_SH_std[t_idx][k] = (samples->L_SH_std[t_idx][k] / samples->bins);
            samples->L_HS_std[t_idx][k] = (samples->L_HS_std[t_idx][k] / samples->bins);
        }
    }
}
