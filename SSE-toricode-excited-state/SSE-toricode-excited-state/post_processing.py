...
def read_sse_output(filename):
    ...
    with open(filename, "r") as file:
        ...
        sampled["Ov"] = np.zeros(sim_info["n_betas"])
        sampled["Ov_std"] = np.zeros(sim_info["n_betas"])
        ...
        for j in range(sim_info["n_betas"]):
            ...
            sampled["E"][j], sampled["E_std"][j], \
            sampled["Ov"][j], sampled["Ov_std"][j], \
...
def read_exact_output(filename):
    ...
    with open(filename, "r") as file:
        ...
        for j in range(sim_info["n_betas"]):
            ...
            sampled["Ov"][j], sampled["m"][j], sampled["m2"][j], sampled["ms"][j], \
